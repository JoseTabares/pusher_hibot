 ...
 * PusherPlugin
 */
public class PusherHibotPlugin implements MethodCallHandler {
    ...
    public static void registerWith(Registrar registrar) {
        ...
        channel.setMethodCallHandler(new PusherHibotPlugin());
        eventStream.setStreamHandler(new EventChannel.StreamHandler() {
            ...
            public void onListen(Object args, final EventChannel.EventSink eventSink) {
                PusherHibotPlugin.eventSink = eventSink;
            }
        ...
        new Handler(Looper.getMainLooper()).post(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    PusherHibotPlugin.eventSink.success(eventStreamMessageJson.toString());

                    if (PusherHibotPlugin.isLoggingEnabled) {
                        Log.d(PusherHibotPlugin.TAG, String.format("onEvent: \nCHANNEL: %s \nEVENT: %s \nDATA: %s", channel, event, data));
                    }
                } catch (Exception e) {
                    onError(e);
                }
        ...
        new Handler(Looper.getMainLooper()).post(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    if (PusherHibotPlugin.isLoggingEnabled) {
                        Log.d(PusherHibotPlugin.TAG, "onError : " + e.getMessage());
                        e.printStackTrace();
                    }
                } catch (Exception ex) {
                    if (PusherHibotPlugin.isLoggingEnabled) {
                        Log.d(PusherHibotPlugin.TAG, "onError exception: " + e.getMessage());
                        ex.printStackTrace();
                    }
                }
            }
